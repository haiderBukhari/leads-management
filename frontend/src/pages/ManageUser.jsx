...
export default function ManageUser() {
            ...
            .then(response => {
                ...
                const formattedRows = dataFromBackend.map(item =>
                    createData(
                        ...
                        item.generalManagerName ? item.generalManagerName : item.managerName ? item.managerName : '-',
                        item.file
                    )
                );
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelected = visibleRows.map((n) => n.id); // Select only visible rows
            setSelected(newSelected);
            setSelectedFull(visibleRows); // Update selected full
            return;
        }
        setSelected([]);
        setSelectedFull([]);
